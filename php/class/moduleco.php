...
class Modulco
{
    ...
    public function __construct()
    {
        try {
            ...
            $bdd = new PDO("mysql:host = localhost ;dbname=boutique_en_ligne", 'root', 'root');
        ...
        if (!empty($_nom) && !empty($_prenom) && !empty($_email) && !empty($_adresse) && !empty($_password) && !empty($_passwordConfirm)) {
            ...
            if ($sidoublon == 0) {
                ...
                if ($_passwordConfirm == $_password) {
                    echo 'ok';
                    $insertinto = $bdd->prepare("INSERT INTO user(nom, prenom, email, adresse, password, id_droit) VALUES(?, ?, ?, ?, ?, 1)");
